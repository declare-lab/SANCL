...
data_dir = f"/home/wh.347426/dataset/lazada/{cat}/set{set_num}"
dill_dir = f"{data_dir}/save"
dst_dir = f"{data_dir}/save_new"
...
def main():
    ...
    for split in ['train', 'dev', 'test']:
        ...
        for idx, row in tqdm(dp.frame[:].iterrows()):
            ...
            try:
                ...
                for i in range(len(tks)):
                    if str(tks[i]) != rvw_text_tokens[i]:
                        ...
                        break
                # import ipdb; ipdb.set_trace()
                mask = np.array([False for _ in range(len(rvw_text_idx))])
            ...
            assert len(mask) == len(rvw_text_idx)
