...
def process_data(data_file,
    ...
    for products, reviews in tqdm(parse_aligned(data_file)):
        product_id = str(products[0]['asin'])
        ...
        for k, v in extract_product_key.items():
            product_frame[k].append(product.get(v, ""))
        ...
        for idx, review in enumerate(reviews):
            ...
            for k, v in extract_review_key.items():
                reviews_frame[k].append(review.get(v, ""))
            upvotes.append(int(review['vote'].replace(',', '').replace(' ', '')))
...
if __name__ == '__main__':
    ...
    set_num = 2
    align_file = f"/home/wh.347426/dataset/amazon18/{cat}/set{set_num}/{cat}.align"
