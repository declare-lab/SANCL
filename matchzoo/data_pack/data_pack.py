...
class DataPack(object):
    ...
    def save(self, dirpath: typing.Union[str, Path], name: str):
        """
        ...
        data_file_path = dirpath.joinpath('%s.%s' % (name, DataPack.DATA_FILENAME))
    ...
    def drop_empty(self):
        ...
        empty_left_id = self._left[
            self._left['text_left_length'] == 0].index.tolist()
        empty_right_id = self._right[
            self._right['text_right_length'] == 0].index.tolist()
    ...
    @_optional_inplace
    def drop_field_empty(self, field: str, mode: str):
        """
        Process empty data by removing corresponding rows.
        ...
        :param inplace: `True` to modify inplace, `False` to return a modified
            copy. (default: `False`)
        """
        check_name = '%s_length' % field

        if mode == 'left':
            empty_left_id = self._left[
                self._left[check_name] == 0].index.tolist()
            empty_id = self._relation[
                self._relation['id_left'].isin(empty_left_id)].index.tolist()
            self._left = self._left.drop(empty_left_id)
        elif mode == 'right':
            empty_right_id = self._right[
                self._right[check_name] == 0].index.tolist()
            empty_id = self._relation[
                self._relation['id_right'].isin(empty_right_id)].index.tolist()
            self._right = self._right.drop(empty_right_id)
        elif mode == 'both':
            empty_left_id = self._left[
                self._left[check_name] == 0].index.tolist()
            ...
            empty_id = self._relation[
                self._relation['id_left'].isin(empty_left_id) | self._relation[
                    'id_right'].isin(empty_right_id)].index.tolist()
            self._left = self._left.drop(empty_left_id)
            self._right = self._right.drop(empty_right_id)
        else:
    ...
    def append_text_length(self, verbose=1):
        ...
        self.apply_on_text(len, rename=('text_left_length', 'text_right_length'),
                           inplace=True, verbose=verbose)

    @_optional_inplace
    def append_field_length(self, field: str, mode: str = 'both', verbose=1):
        """
        Append `length_left` and `length_right` columns.
        ...
        """
        rename = (field + '_length', field +
                  '_length') if mode == 'both' else field + '_length'
        self.apply_on_field(len, field, mode, rename,
                            inplace=True, verbose=verbose)
    ...
    def apply_on_field(
        ...
        """
        if mode == 'both':
            if field in set(self.left.columns) and field in set(self.right.columns):
                self._apply_on_field_both(func, field, rename, verbose=verbose)
            else:
                raise ValueError(f"{field} does not exist in both Dataframes.")
        elif mode == 'left':
            if field in set(self.left.columns):
                self._apply_on_field_left(func, field, rename, verbose=verbose)
            else:
                raise ValueError(
                    f"{field} does not exist in the left Dataframe.")
        elif mode == 'right':
            if field in set(self.right.columns):
                self._apply_on_field_right(
                    func, field, rename, verbose=verbose)
            else:
                raise ValueError(
                    f"{field} does not exist in the right Dataframe.")
        else:
            raise ValueError(f"{mode} is not a valid mode type."
                             f"Must be one of `left` `right` `both`.")
    ...
    def _apply_on_field_right(self, func, field, rename, verbose=1):
        name = rename or field
        if verbose:
            tqdm.pandas(desc="Processing " + name + " with " + func.__name__)
            self._right[name] = self._right[field].progress_apply(func)
        else:
            self._right[name] = self._right[field].apply(func)

    def _apply_on_field_left(self, func, field, rename, verbose=1):
        name = rename or field
        if verbose:
            tqdm.pandas(desc="Processing " + name + " with " + func.__name__)
            self._left[name] = self._left[field].progress_apply(func)
        else:
            self._left[name] = self._left[field].apply(func)

    def _apply_on_field_both(self, func, field, rename, verbose=1):
        left_name, right_name = rename or (field, field)
        self._apply_on_field_left(
            func, field, rename=left_name, verbose=verbose)
        self._apply_on_field_right(
            func, field, rename=right_name, verbose=verbose)
    ...
    @staticmethod
    def load(dirpath: typing.Union[str, Path], name: str):
        """
