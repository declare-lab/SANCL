...
class XLMRHPTaskCallback(BaseCallback):
    """
    Pad and construct data for XLM RHP task.
    ...
    def __init__(
        ...
        self._pad_word_value = pad_word_value
        self._cls_word_value = cls_word_value
        self._sep_word_value = sep_word_value
        self._lang_id_value = lang_id_value
        self._add_cls = add_cls
        self._add_sep = add_sep
        self._sequence_a_segment_id = sequence_a_segment_id
        self._sequence_b_segment_id = sequence_b_segment_id
        self._cls_token_segment_id = cls_token_segment_id
        self._pad_token_segment_id = pad_token_segment_id
        
        self._pad_args = list(zip(text_fields, fixed_length, pad_word_value))

    def on_batch_unpacked(self, x: dict, y: np.ndarray):
        concate_x = self._concate_sequence(x)

        # batch size
        batch_size = len(concate_x['input_ids'])

        # max length
        pad_length = int(max(x['length']))
        if self._fixed_length:
            pad_length = self._fixed_length

        # pad
        x.clear()
        concate_x.pop('length')
        for f in concate_x.keys():
            value = x[f]
            dtype = _infer_dtype(value)

            if f == 'input_ids':
                pad_value = self._pad_word_value
            else:
                pad_value = 0
            
            padded_value = np.full([batch_size, pad_length], pad_value, dtype=dtype)
            _padding_2D(value, padded_value, pad_value)
            x[f] = padded_value

    def _concate_sequence(self, x: dict):
        """
        The input x is a dict containing different indexs in the left and right datpack.
        In the Instance dataloader, all the input is a list by list typing.

        For example:
        >>> x['text_left'] = [[...], ..., [...]]
        >>> x['text_right'] = [[...], ..., [...]]
        >>> x['text_left_length'] = [3, ..., 7]
        >>> x['text_right_length'] = [3, ..., 7]

        We need to concate `text_right` and `text_left` as a whole input sequnce to the
        transformers and also constructing some necessary components.
        """
        zip_instance = zip(x[f] for f in self._text_fields)

        concate_x = defaultdict(list)
        for insts in zip_instance:
            input_ids = []
            segment_ids = []
            
            # add cls
            if self._add_cls:
                input_ids.append(self._cls_word_value)
                segment_ids.append(self._cls_token_segment_id)
        
            # add first sequence
            input_ids.extend(insts[0])
            segment_ids.extend([self._sequence_a_segment_id] * len(insts[0]))
            
            # add remains sequence
            for ins in insts[1:]:
                if self._add_sep:
                    input_ids.append(self._sep_word_value)
                    segment_ids.append(self._sequence_b_segment_id)
                input_ids.extend(ins)
                segment_ids.extend([self._sequence_b_segment_id] * len(ins))

            input_mask = [1 if self._mask_padding_with_zero else 0] * len(input_ids)
            lang_ids = [self._languages_id] * len(input_ids)

            # check
            length = len(input_ids)
            assert len(segment_ids) == length
            assert len(input_mask) == length
            assert len(lang_ids) == length

            # collect
            concate_x['input_ids'].append(input_ids)
            concate_x['segment_ids'].append(segment_ids)
            concate_x['input_mask'].append(input_mask)
            concate_x['lang_ids'].append(lang_ids)
            concate_x['length'].append(length)


class TransformersPadding(BaseCallback):
    """
    Pad and construct data for Transformers task.
    """
    ...
    def __init__(
        ...
        text_fields: typing.List[str],
        max_length: typing.Union[int, list] = 128,
        padding: typing.Union[str, bool] = 'max_length',
        truncation: typing.Union[str, bool] = True
    ):
        """Init."""
        self._text_fields = text_fields
        ...
        self._max_length = max_length if isinstance(max_length, list) else [max_length]
