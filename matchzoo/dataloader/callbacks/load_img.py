...
class LoadImageField(LoadImage):
    def __init__(
        self,
        image_fields: dict,
        field_is_padded: dict,
        feat_dir: str,
        feat_size: int,
        max_roi_num: int,
        img_min_length: int,
        ...
        self.field_is_padded = field_is_padded
        self.feat_dir = feat_dir
        self.feat_size = feat_size
        self.max_roi_num = max_roi_num
        self.img_min_length = img_min_length
    ...
    def on_batch_unpacked(self, x: dict, y: np.ndarray):
        "Load related image features, instance without image will be padded"
        for src_k, map_k in self.image_fields.items():
            src_id = x[src_k]
            max_pref = min(self.max_prefetch, len(src_id))
            image_map, image_map_length = self.load_images(src_id, max_pref, map_k)
            x[map_k] = image_map
            x[f"{map_k}_length"] = image_map_length

    def load_image_per_instance(self, dir_names, map_field):
        for dir_name in dir_names:
            files_dir = os.path.join(self.feat_dir, dir_name)
            pth_path = os.path.join(files_dir, f"{dir_name}.pth")
            if not os.path.exists(pth_path) or self.field_is_padded.get(map_field, False):
                yield [self._placeholder_img()]
            else:
                npz_files = torch.load(pth_path)
                feat = []
    ...
    def load_images(self, dir_names, max_pref, map_field):
        image = []
        length = []
        for npz_list in BackgroundGenerator(self.load_image_per_instance(dir_names, map_field), max_prefetch=max_pref):
            # concate feats
            feats = np.concatenate(npz_list, axis=0)
            image.append(feats)
            length.append(feats.shape[0])
