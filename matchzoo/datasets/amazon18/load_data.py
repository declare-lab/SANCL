...
from .dataprocess import _read_data, _read_data2, _read_data3, _read_data4, _read_data5


def load_data(read_type, feature_root, *args, **kwargs):
    if read_type == 'all':
        read_func = _read_data3
    elif read_type == 'image':
        read_func = _read_data4
    elif read_type == 'image_product':
        read_func = _read_data5
    else:
        raise ValueError(f"Invalid read type {read_type} in (`all`, `image`).")
