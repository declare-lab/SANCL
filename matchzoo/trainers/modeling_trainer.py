...
class ModelingTrainer(Trainer):
    ...
    def _run_epoch(self):
        ...
        cpc_score = AverageMeter()

        train_loss_list = []
        cpc_score_list = []

        model_p = filter(lambda p: p.requires_grad, self._model.parameters())
        params = sum([np.prod(p.size()) for p in model_p])
        ...
        with tqdm(enumerate(self._trainloader), total=num_batch,
            ...
            for step, (inputs, target) in pbar:
                ...
                outputs, cpc = self._model(inputs)
                ...
                cpc_score.update(cpc.item())
              
                # model_p = filter(lambda p: p.grad is not None, self._model.parameters())
                # params = sum([np.prod(p.size()) for p in model_p])
                ...
                if step % 10 == 0:
                    ...
                    f'Epoch {self._epoch}/{self._epochs} Loss {train_loss.avg:.3f} CPC {cpc_score.avg: .3f}')
                    train_loss_list.append(train_loss.avg)
                    cpc_score_list.append(cpc_score.avg)
                    train_loss.reset()
                    cpc_score.reset()
        ...
        cpc_scores = np.array(cpc_score_list)
